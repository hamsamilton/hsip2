...
rr_72 <- rr %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:80) 

# check counts 
View(rr_72 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(rr_72$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
rr_72 <- rr_72 %>% group_by(subject_id) %>% 
...
rr_72 <- merge(rr_72, m_ent, by = "subject_id")

# save as csv
write.csv(rr_72, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/rr_72.csv", row.names = FALSE)
