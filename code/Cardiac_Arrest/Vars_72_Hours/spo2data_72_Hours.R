...
spo2_72 <- spo2 %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:80) 

# check counts 
View(spo2_72 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(spo2_72$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
spo2_72 <- spo2_72 %>% group_by(subject_id) %>% 
...
spo2_72 <- merge(spo2_72, m_ent, by = "subject_id")

# save as csv
write.csv(spo2_72, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/spo2_72.csv", row.names = FALSE)
