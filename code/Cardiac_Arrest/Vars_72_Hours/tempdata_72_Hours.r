...
CalcEnSlide <- function(entmeasure = "FastSampEn",df,grouplist,r_thrsh = .2,size = 50, step = 1) {
...
temp_72 <- temp %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:50) 

# check counts 
View(temp_72 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(temp_72$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
temp_72 <- temp_72 %>% group_by(subject_id) %>% 
...
temp_72 <- merge(temp_72, m_ent, by = "subject_id")

# save as csv
write.csv(temp_72, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/Temp_72.csv", row.names = FALSE)
