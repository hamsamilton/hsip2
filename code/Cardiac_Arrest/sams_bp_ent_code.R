...
CalcEn <- function(entmeasure = "FastSampEn",df,varofint,grouplist,r_thrsh = .2) {
  k <- ddply(df,
             grouplist,
             here(summarise),
             entropy = do.call(entmeasure,list(varofint,
                                               r = r_thrsh*sd(varofint))
             )
  )
  # names(k)[5] <- "N"
  return(k)
...
sbp_counts <- sbp %>% group_by(hadm_id) %>% count(num_obs = n())

sbp_counts_200 <- sbp_counts %>% filter(num_obs >= 310)
sbp <- sbp %>% 
