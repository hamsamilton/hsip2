...
rr <- merge(rr, admissions, by = c("subject_id", "hadm_id"))
...
# Select the first 100 measurements for each patients
rr_96 <- rr %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:100) 

# check counts 
View(rr_96 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(rr_96$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 100 measurements
library(dplyr) #reload, interferring with plyr
rr_96 <- rr_96 %>% group_by(subject_id) %>% 
...
rr_96 <- merge(rr_96, m_ent, by = "subject_id")

# save as csv
write.csv(rr_96, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/rr_96.csv", row.names = FALSE)
