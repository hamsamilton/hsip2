...
bp_96 <- bp %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:100) 

# check counts 
View(bp_96 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(bp_96$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
bp_96 <- bp_96 %>% group_by(subject_id) %>% 
...
bp_96 <- merge(bp_96, m_ent, by = "subject_id")

# save as csv
write.csv(bp_96, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/bp_96.csv", row.names = FALSE)
