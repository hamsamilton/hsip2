...
hr_96 <- hr %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:100) 

# check counts 
View(hr_96 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(hr_96$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
hr_96 <- hr_96 %>% group_by(subject_id) %>% 
...
hr_96 <- merge(hr_96, m_ent, by = "subject_id")

# save as csv
write.csv(hr_96, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/hr_96.csv", row.names = FALSE)
