...
temp <- merge(temp, admissions, by = c("subject_id", "hadm_id"))
...
#plot_temp(29769) # high entropy - deceased
#plot_temp(27850) # high entropy - not deceased
#plot_temp(10993) # low entrop, not deceased

# Select the first 80 measurements for each patients
temp_96 <- temp %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:80) 

# check counts 
View(temp_96 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(temp_96$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
temp_96 <- temp_96 %>% group_by(subject_id) %>% 
...
temp_96 <- merge(temp_96, m_ent, by = "subject_id")

# save as csv
write.csv(temp_96, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/Temp_96.csv", row.names = FALSE)
