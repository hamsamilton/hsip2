...
plot_hr(28075) #low ent - not deceased
plot_hr(3046) # high entropy - not deceased
plot_hr(9825) # high entropy - deceased

# Select the first 80 measurements for each patients
hr_80 <- hr %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:80) 

# check counts 
View(hr_80 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(hr_80$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
hr_80 <- hr_80 %>% group_by(subject_id) %>% 
...
hr_80 <- merge(hr_80, m_ent, by = "subject_id")

# save as csv
write.csv(hr_80, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/hr_80.csv", row.names = FALSE)
