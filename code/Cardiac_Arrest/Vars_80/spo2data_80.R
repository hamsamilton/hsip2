...
# both deceased
plot_spo2(4331) #low ent
plot_spo2(3139) # high entropy - seems like a huge outlier!

# Select the first 80 measurements for each patients
spo2_80 <- spo2 %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:80) 

# check counts 
View(spo2_80 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(spo2_80$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
spo2_80 <- spo2_80 %>% group_by(subject_id) %>% 
...
spo2_80 <- merge(spo2_80, m_ent, by = "subject_id")

# save as csv
write.csv(spo2_80, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/spo2_80.csv", row.names = FALSE)
