...
plot_bp(12245) #high entropy - deceased
plot_bp(29497) # high entropy - not deceased

# deceased
plot_bp(11952) # low entropy - not deceased

# Select the first 80 measurements for each patients
bp_80 <- bp %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:80) 

# check counts 
View(bp_80 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(bp_80$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 80 measurements
library(dplyr) #reload, interferring with plyr
bp_80 <- bp_80 %>% group_by(subject_id) %>% 
...
bp_80 <- merge(bp_80, m_ent, by = "subject_id")

# save as csv
write.csv(bp_80, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/bp_80.csv", row.names = FALSE)
