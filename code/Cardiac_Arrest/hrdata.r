...
hr_counts <- hr %>% group_by(subject_id) %>% dplyr::count(num_obs = n())
hr_counts_200 <- hr_counts %>% filter(num_obs > 200) #(specify num_obs > 200 to avoid error with the entropy function!)
hr <- hr %>% filter(subject_id %in% hr_counts_200$subject_id)
...
plot_hr <- function(x) { 
  ...
  hr <- df %>% filter(!is.na(value)) %>% select(subject_id, charttime_conv, value) %>%
    ...
    #geom_hline(yintercept=3) + #could demark dangerous (low or high) hr
...
plot_hr(7911) #high ent 
plot_hr(6204) # low entropy 

# Select the firest 200 measurements for each patients
hr_200 <- hr %>% group_by(subject_id) %>% 
  arrange(charttime_conv) %>% 
  slice(1:200) 

# check counts 
View(hr_200 %>% group_by(subject_id) %>% dplyr::summarize(count = n()))
nrow(unique(data.frame(hr_200$subject_id)))

# Determine the max, min, mean, sd, and variance of the first 200 measurements
library(dplyr) #reload, interferring with plyr
hr_200 <- hr_200 %>% group_by(subject_id) %>% 
...
hr_200 <- merge(hr_200, m_ent, by = "subject_id")

# save as csv
write.csv(hr_200, "/Users/User/Box Sync/Projects/Mimic_HSIP/Mimic_Data/Cardiac_Arrest/hr_200.csv", row.names = FALSE)
